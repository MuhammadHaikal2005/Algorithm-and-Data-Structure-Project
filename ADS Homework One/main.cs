...
class HelloWorld
{
    static void Main()
    {
        ...
        {
            file.WriteLine("NumDrones,AverageTime,BubbleSortTime,PrintTime,MaxTime");
            for (int i = 0; i < numsteps; i++)
            {
                int numdrones = i * stepsize + min;
                file.WriteLine($"{numdrones},{timeAverage[i]},{timeBubbleSort[i]},{timePrint[i]},{timeMax[i]}");
            }
        }
